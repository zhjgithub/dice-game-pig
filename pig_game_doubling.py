...
def strategy_d(state):
    ...
    if double == 'double':
        return 'decline' if you + pending >= goal else 'accept'
    if double > 1:
        return 'hold' if pending >= 20 or me + pending >= goal else 'roll'
    return 'double' if me + pending >= goal else 'hold' if pending >= 20 else 'roll'


def hold_20_d(state):
    "Hold at 20 pending.  Always accept; never double."
    (p, me, you, pending, double) = state
    return ('accept' if double == 'double' else 'hold'
            if (pending >= 20 or me + pending >= goal) else 'roll')
    ...
    if action not in pig_actions_d(state):
        ...
        if d == 1:
            return (other[p], you, me + 1, 0,
                    double)  # pig out; other player's turn
        else:
            return (p, me, you, pending + d,
                    double)  # accumulate die in pending
    elif action == 'hold':
...
def strategy_compare(A, B, N=1000):
    ...
    print(
        'In %s games of pig, strategy %s took %s percent of the points against %s.'
...
def test():
    ...
    assert set(pig_actions_d((0, 5, 5, 5, 1))) == set(
        ['roll', 'hold', 'double'])
    assert set(pig_actions_d((1, 10, 15, 6, 'double'))) == set(
        ['accept', 'decline'])
    assert strategy_compare(strategy_d,
                            hold_20_d) > 60  # must win 60% of the points
    print('test passes')
